...
// The "no-image" visual: a per-category Pulse glyph seeded from the title.
// The generator is chosen BY category so the mark encodes what the project is.
function PulseGlyph({ category, seed }) {
  ...
  if (/back/i.test(category)) {
    ...
    return (
      <svg viewBox={`0 0 ${W} ${H}`} className="h-[60px] w-[132px] overflow-visible">
        <line x1="0" y1={H / 2} x2={W} y2={H / 2} stroke="var(--color-abyss-line)" strokeWidth="1" />
        <motion.polyline
          points={pts.join(" ")}
          fill="none"
  ...
  // ML / Computer Vision → dot-field with dashed pulse detection boxes.
    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="h-[60px] w-[132px] overflow-visible">
      ...
      {nodes.map((nd, i) => (
        <motion.circle
          ...
          cy={nd.y}
          r={nd.lit ? 2.4 : 1.6}
          fill={nd.lit ? "var(--color-pulse)" : "var(--color-frost-dim)"}
          initial={{ opacity: 0, scale: 0 }}
          animate={{ opacity: nd.lit ? 1 : 0.7, scale: 1 }}
...
// desktop, tap/focus on touch+keyboard) it indents, lights pulse, dims its
...
function WorkRow({ project, index, active, dimmed, isLive, onActivate, onClear, onToggle }) {
  ...
  return (
    <li
      className={`relative border-t border-abyss-line transition-opacity duration-300 ${
    ...
    >
      ...
      <span
        ...
        className={`absolute left-0 top-0 h-full w-0.5 origin-bottom transition-transform duration-300 ${
          active ? "scale-y-100 bg-pulse" : project.featured ? "scale-y-100 bg-pulse/30" : "scale-y-0 bg-pulse/30"
        }`}
      />
      ...
      >
        ...
        <span
          className={`font-mono text-lg tabular-nums transition-colors duration-300 ${
            active ? "text-pulse" : "text-abyss-line"
          }`}
        >
          {num}
        </span>
        ...
        <span className="min-w-0">
          <span className="flex items-center gap-2.5">
            {project.featured && <span aria-hidden="true" className="h-1.5 w-1.5 shrink-0 rounded-full bg-pulse" />}
            <span
              className={`truncate font-display-3 text-2xl font-bold tracking-tight transition-colors duration-300 md:text-3xl ${
                active ? "text-frost" : "text-frost/85"
              }`}
            >
              {project.title}
            </span>
          ...
          <span className="mt-1.5 block font-mono text-[11px] uppercase tracking-wider text-frost-dim md:hidden">
            {project.name} · {project.category} · {project.year}
          </span>
        </span>

        {/* ORG / CATEGORY / YR — desktop columns */}
        <span className="hidden font-mono text-sm text-frost-dim md:block">{project.name}</span>
        <span className="hidden font-mono text-[11px] uppercase tracking-wider text-frost-dim md:block">
          {project.category}
        </span>
        <span className="hidden font-mono text-sm tabular-nums text-frost-dim md:block">{project.year}</span>

        {/* Arrow */}
        <span
          className={`justify-self-end text-lg transition-all duration-300 ${
            active ? "translate-x-1 text-pulse" : "text-abyss-line"
          }`}
        >
          →
        </span>
        ...
        {active && (
          ...
          >
            <div className="grid grid-cols-1 gap-6 pb-7 pl-4 pr-2 md:grid-cols-[minmax(0,1fr)_auto] md:items-end md:gap-10 md:pl-[3.5rem]">
              <div>
                <p className="max-w-[60ch] leading-relaxed text-frost">{project.description}</p>
                <span
                  className={`mt-4 inline-flex items-center gap-2 font-mono text-xs uppercase tracking-wider ${
                    isLive ? "text-pulse" : "text-frost-dim"
                  }`}
                >
                  {isLive ? "View case →" : "Internal · production"}
                </span>
              </div>
              <div className="shrink-0 rounded-xl border border-abyss-line bg-abyss p-3">
                <PulseGlyph category={project.category} seed={project.title} />
              </div>
            </div>
          </motion.div>
        )}
      ...
      <div className="container-px">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex max-w-sm flex-col gap-3">
            ...
            <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-frost-dim">
        ...
        <motion.div
          ...
          className="mt-14 overflow-hidden rounded-2xl border border-abyss-line bg-abyss-soft"
        >
          {/* Spec-sheet column header (desktop) */}
          <div className="hidden grid-cols-[3rem_minmax(0,1fr)_10rem_10rem_3.5rem_1.75rem] gap-x-6 px-5 py-4 font-mono text-[10px] uppercase tracking-[0.2em] text-frost-dim md:grid">
            <span>No.</span>
            <span>Project</span>
            <span>Org</span>
            <span>Category</span>
